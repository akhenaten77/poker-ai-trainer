...
import { playChipSound, playCardSlideSound, playCardFlipSound, playFoldSound } from "../../lib/sounds";

export default function PlayPage() {
  const store = useGameStore();
  const prevRoundRef = useRef(store.currentRound);
  const prevPlayerRef = useRef(store.currentPlayerIndex);
  ...
  // ─── SOUND TRIGGERS ────────────────────────────────────────────────
  // Play card sounds when community cards are revealed (round changes)
  useEffect(() => {
    if (prevRoundRef.current !== store.currentRound) {
      if (['FLOP', 'TURN', 'RIVER'].includes(store.currentRound)) {
        playCardFlipSound();
        // Stagger additional slide sounds for flop (3 cards)
        if (store.currentRound === 'FLOP') {
          setTimeout(() => playCardSlideSound(), 80);
          setTimeout(() => playCardSlideSound(), 160);
        }
      }
  ...
  useEffect(() => {
    ...
    if (currentPlayer && currentPlayer.isBot) {
      ...
      const timer = setTimeout(() => {
        ...
        // Play appropriate sound for bot action
        if (action.action === 'FOLD') playFoldSound();
        else if (action.action === 'RAISE' || action.action === 'CALL') playChipSound();
  ...
  // ─── ACTION HANDLER WITH SOUND ─────────────────────────────────────
  const handleAction = (action: 'FOLD' | 'CHECK' | 'CALL' | 'RAISE') => {
    if (action === 'FOLD') playFoldSound();
    else if (action === 'RAISE' || action === 'CALL') playChipSound();
    store.playerAction(action, store.pot);
  };

  // Render community cards with staggered deal delays
  const renderCommunityCards = () => {
    const cards = [];
    for (let i = 0; i < 5; i++) {
      if (communityCards[i]) {
        cards.push(<PlayingCard key={i} card={communityCards[i]} dealDelay={i * 80} />);
      } else {
        cards.push(<PlayingCard key={i} hidden dealDelay={0} />);
      }
    }
    return cards;
  };
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        <main className="lg:ml-64 flex-1 relative flex flex-col items-center p-4 md:p-8 bg-surface-dim overflow-y-auto overflow-x-hidden w-full">
          <div className="w-full flex flex-col max-w-6xl mx-auto min-h-min justify-center py-10 lg:py-16">
            {/* ═══ POKER TABLE ═══ */}
            <div className="relative w-full min-h-[400px] sm:min-h-[450px] lg:min-h-[500px] poker-felt rounded-[3rem] sm:rounded-[6rem] lg:rounded-[200px] border-[6px] sm:border-[12px] border-[#1a0f08] neon-rim flex flex-col items-center justify-center shrink-0 mb-16 sm:mb-24">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none" style={{ zIndex: 5 }}>
                ...
                <div className="flex items-end gap-3 mb-6">
                  {pot > 0 && (
                    <div className="pointer-events-auto">
                      <ChipStack amount={Math.round(pot)} animate size="sm" />
                    </div>
                  )}
                  <div className="bg-black/50 backdrop-blur-md px-5 py-2 rounded-full border border-primary-container/20 flex flex-col items-center">
                    <span className="text-on-surface-variant text-[9px] uppercase tracking-widest font-headline">Total Pot</span>
                    <span className="text-primary-container font-black text-xl sm:text-2xl font-headline drop-shadow-[0_0_8px_rgba(57,255,20,0.5)]">
                      {pot.toFixed(1)} BB
                    </span>
                ...
                <div className="flex gap-1 sm:gap-2">
                  {renderCommunityCards().map((card, i) => (
                    <div key={i} style={{ zIndex: i + 10 }}>
                      {card}
                    </div>
                  ))}
                </div>
              ...
              {bots.map((bot, idx) => {
                const positions = [
                  "top-0 left-[5%] sm:left-[15%] lg:left-[20%] -translate-y-[30%] sm:-translate-y-[40%]",
                  "top-0 right-[5%] sm:right-[15%] lg:right-[20%] -translate-y-[30%] sm:-translate-y-[40%]",
                  "right-0 top-1/2 translate-x-[20%] sm:translate-x-[40%] -translate-y-1/2",
                  "left-0 top-1/2 -translate-x-[20%] sm:-translate-x-[40%] -translate-y-1/2",
                ];
                ...
                return (
                  <div key={bot.id} className={`absolute ${positions[idx]} flex flex-col items-center pointer-events-none transition-all z-10 ${isActive ? 'scale-100 sm:scale-110' : 'scale-85 sm:scale-100'}`}>
                    <div className={`bg-surface-container-high/90 backdrop-blur-sm rounded-2xl p-2 border ${isActive ? 'border-primary-container shadow-[0_0_15px_rgba(57,255,20,0.4)]' : 'border-outline-variant/50'} w-24 sm:w-28 text-center shadow-lg transition-all`}>
                    ...
                    {bot.hasFolded ? (
                      ...
                      <div className="mt-2 flex items-end gap-1.5 pointer-events-auto">
                        <ChipStack amount={Math.round(bot.currentBet)} animate size="sm" />
                        <span className="text-[10px] text-primary-fixed-dim font-bold bg-black/60 px-2 py-0.5 rounded-full border border-primary-fixed-dim/30">
                          {bot.currentBet.toFixed(1)}
                        </span>
                      </div>
                    ) : (
                      <div className="mt-2 bg-black/40 px-3 py-1 rounded-full text-[10px] text-on-surface-variant/60 border border-outline-variant/20">
                        —
                      </div>
                    )}
                  </div>
              ...
              <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-[35%] sm:translate-y-[45%] flex flex-col items-center z-10 transition-all ${store.currentPlayerIndex === 0 ? 'scale-105 sm:scale-110' : 'scale-90 sm:scale-100'}`}>
                {/* Hero Cards — fanned with slight rotation */}
                <div className="flex mb-2 relative scale-90 sm:scale-100">
                  {hero.cards.map((card, i) => (
                    <div
                      key={i}
                      className="relative transition-transform duration-200 hover:z-[60]"
                      style={{
                        zIndex: i + 10,
                    ...
                    >
                      <PlayingCard card={card} dealDelay={i * 120} />
                    </div>
                  ))}
                </div>
                {/* Hero chip bet */}
                {hero.currentBet > 0 && (
                  <div className="flex items-end gap-1.5 mb-1">
                    <ChipStack amount={Math.round(hero.currentBet)} animate size="sm" />
                    <span className="text-[10px] text-primary-fixed-dim font-bold bg-black/60 px-2 py-0.5 rounded-full">
                      {hero.currentBet.toFixed(1)}
                    </span>
                  </div>
                )}
                {/* Hero Info */}
